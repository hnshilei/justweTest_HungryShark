...
public class BaseSprite extends BaseSub {
    ...
    private float px =0, py=0;//旋转中心坐标
    ...
    private void init() {
        s_alpha = 255;
        s_canvas = null;
        s_texture = new GameTexture(s_engine.getActivity());
        s_paint = new Paint();
    ...
    public void drawWithFixedFrame() {
        ...
        // scale
        int w = (int)(s_width);// * Math.sqrt(s_scale.x)
        int h = (int)(s_height);// * Math.sqrt(s_scale.y)
        s_dst.set(0, 0, w, h);
        // draw the frame
    ...
    public void setRotationAnchor(float px, float py) {
        this.px = px;
        this.py = py;
    }
    ...
    public RectF getBounds() {
        ...
        return new RectF((int) s_position.x, (int) s_position.y,
                ...
                (int) (s_position.y + s_height * Math.sqrt(s_scale.y)));
    }
