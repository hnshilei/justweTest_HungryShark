...
public class TextureButton extends BaseButton {
    ...
    private void init() {
        ...
        zoomCenter = false;
        firstInit = false;
        texture = new GameTexture(b_engine.getActivity());
...
//      e_canvas.drawBitmap(texture.getBitmap(), src, b_rect, paint);
        ...
        s_mat_scale.setScale(b_scale.x, b_scale.y);
        s_mat_rotate.setRotate((float) Math.toDegrees(b_rotation),px,py);
        s_mat_translation.setTranslate(b_position.x, b_position.y);

        s_matrix.postConcat(s_mat_scale);
        s_matrix.postConcat(s_mat_rotate);
        s_matrix.postConcat(s_mat_translation);
        
        e_canvas.drawBitmap(texture.getBitmap(), s_matrix, paint);
